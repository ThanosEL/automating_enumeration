...
# Add Go bin to PATH in both .zshrc and .bashrc
add_to_path() {
  local RC_FILE="$1"
  local ENTRY="$2"
  [[ -f "$RC_FILE" ]] || return
  grep -qF "$ENTRY" "$RC_FILE" 2>/dev/null || echo "export PATH=\$PATH:$ENTRY" >> "$RC_FILE"
}

add_to_path "$HOME/.zshrc"  '$HOME/go/bin'
add_to_path "$HOME/.bashrc" '$HOME/go/bin'
...
add_to_path "$HOME/.zshrc"  '$HOME/.cargo/bin'
add_to_path "$HOME/.bashrc" '$HOME/.cargo/bin'
